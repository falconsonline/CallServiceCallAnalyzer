...
def generate_transaction_html(flow_records, html_path, display_id,
    ...
    def _mermaid(tx_flows: list, next_flows: list = None) -> str:
        ...
        # Rightmost participant ID for flow-jump note placement (right side of diagram)
        _marker_cs       = _sanitize_id(cs_names[-1]) if cs_names else 'CallService'
        _prev_ts_sec     = -1.0
        _pending_markers = sorted(next_flows or [], key=lambda x: x['ts_sec'])

        for r in sorted(tx_flows, key=lambda x: _ts_to_sec(x.get('timestamp', ''))):
            _curr_ts_sec = _ts_to_sec(r.get('timestamp', ''))
            # Emit any pending markers whose next-flow ts falls between prev and current.
            # Sentinel is short (≤8 chars) so Mermaid never wraps it.
            _still_pending = []
            for _nf in _pending_markers:
                if _prev_ts_sec < _nf['ts_sec'] <= _curr_ts_sec:
                    lines.append(
                        f"    Note right of {_marker_cs}: ⬇flow-{_nf['idx']}"
                    )
                else:
                    _still_pending.append(_nf)
            _pending_markers = _still_pending
            ...
            _prev_ts_sec = _curr_ts_sec

        # Fallback: any markers whose next-flow starts after all messages in this flow
        for _nf in _pending_markers:
            lines.append(f"    Note right of {_marker_cs}: ⬇flow-{_nf['idx']}")
...
<style>
  ...
  .flow-jump-link:hover {{ opacity:1; }}
...
""".format(display_id=display_id)
    ...
    # Pre-compute next-flow info: only the FIRST flow gets markers for ALL subsequent flows
    _tx_keys = list(transactions.keys())
    _all_next_flows: list = []
    for _nfi, _nfk in enumerate(_tx_keys[1:], 1):
        _best_sec = float('inf')
        _best_ts  = ''
        for _r in transactions[_nfk]:
            _s = _ts_to_sec(_r.get('timestamp', ''))
            if _r.get('timestamp') and _s < _best_sec:
                _best_sec = _s
                _best_ts  = _r.get('timestamp', '')
        _disp = _best_ts.strip()
        if 'T' in _disp:
            _disp = _disp.split('T', 1)[1]
        _disp = _disp.split(' ')[-1][:12]
        _all_next_flows.append({
            'idx':        _nfi,
            'label':      _nfk,
            'ts_sec':     _best_sec,
            'ts_display': _disp,
        })
    # Map: first key → all subsequent flows; all others → empty
    _next_flows_map: dict = {_tx_keys[0]: _all_next_flows} if len(_tx_keys) > 1 else {}
    ...
    for t_idx, (tx_key, tx_flows) in enumerate(transactions.items()):
        ...
        mmd      = _mermaid(tx_flows, next_flows=_next_flows_map.get(tx_key, []))
        ...
        _fsmid_tag = (f' <span style="font-size:.8em;font-weight:normal;'
                      ...
                      if _tx_fsmid else '')
        _back_btn = ('<button class="copy-btn" onclick="returnToFlow()">&#8592; Back</button>'
                     if t_idx > 0 else '')
        # Embed next-flow timestamp data on the wrapper so JS doesn't need to parse SVG text
        import json as _json
        _flow_nfs = _next_flows_map.get(tx_key, [])
        _fj_attr  = (f' data-flow-jumps=\'{_json.dumps([{"idx":nf["idx"],"ts":nf["ts_display"]} for nf in _flow_nfs])}\''
                     if _flow_nfs else '')
        html += f"""
  <div class="tx-box" id="flow-{t_idx}">
    <div class="tx-header">
      <h2>Transaction: {safe_key}{_fsmid_tag}</h2>
      <span class="tx-actions">{_back_btn}<button class="copy-btn" onclick="window.scrollTo({{top:0,behavior:'smooth'}})">&#8679; Top</button><button class="copy-btn" onclick="copyDiagram(this)">&#128247; Copy</button></span>
    </div>
    <div class="mermaid-wrap"{_fj_attr}>
    <div class="mermaid">
{mmd}
    </div>
    </div>
...
<script>
  ...
  // Inject HTML <a> overlay links at the Y position of each ⬇ Mermaid sentinel note.
  // Timestamps come from data-flow-jumps JSON on .mermaid-wrap (never parsed from SVG text
  // which can be split across tspan lines by Mermaid wrap). The SVG note is hidden after use.
  function processFlowJumps() {
    // Remove stale overlays (e.g. after theme toggle re-render)
    document.querySelectorAll('.flow-jump-link').forEach(el => el.remove());
    document.querySelectorAll('.mermaid-wrap[data-flow-jumps]').forEach(wrap => {
      const svg = wrap.querySelector('svg');
      if (!svg) return;
      let jumps;
      try { jumps = JSON.parse(wrap.dataset.flowJumps); } catch(e) { return; }
      jumps.forEach(jmp => {
        // Locate the short sentinel text element: ⬇flow-N
        const sentinel = '⬇flow-' + jmp.idx;
        let sentinelEl = null;
        svg.querySelectorAll('text').forEach(t => {
          if (!sentinelEl && t.textContent.includes(sentinel)) sentinelEl = t;
        });
        if (!sentinelEl) return;
        // Hide the Mermaid note group (it served only as position anchor)
        let noteG = sentinelEl.parentElement;
        while (noteG && noteG !== svg) {
          if (noteG.tagName === 'g' && noteG.querySelector('rect')) break;
          noteG = noteG.parentElement;
        }
        if (noteG && noteG !== svg) noteG.style.visibility = 'hidden';
        // Map SVG text centre → screen coords → .mermaid-wrap-relative top
        try {
          const bbox = sentinelEl.getBBox();
          ...
          const top = Math.max(4, sc.y - wR.top - 12);
          const a   = document.createElement('a');
          a.href        = '#flow-' + jmp.idx;
          a.className   = 'flow-jump-link';
          a.textContent = '⬇ ' + jmp.ts + ' →';
          a.style.top   = top + 'px';
          a.addEventListener('click', () => {
            sessionStorage.setItem('flowReturnY', String(Math.round(window.scrollY)));
          });
          wrap.appendChild(a);
        } catch(e) {}
      });
  ...
  // Return to the scroll position saved when a flow-jump note was clicked
  function returnToFlow() {
    const y = sessionStorage.getItem('flowReturnY');
    if (y !== null) {
      sessionStorage.removeItem('flowReturnY');
      window.scrollTo({ top: parseInt(y, 10), behavior: 'smooth' });
    } else {
      const el = document.getElementById('flow-0');
...
def main():
    ...
    parser.add_argument("-f", "--trace", action="append", dest="trace", default=None,
                        ...
                        help="[MANDATORY, repeatable] Glob pattern for a trace file group. "
                             ...
                             "--trace 'wsmstrace/WSMSTrace*' --trace 'campaigntrace/CampaignTrace*'")
    ...
    parser.add_argument("-t",  "--tcap", default=None,
                        help="Glob for TcapServer log file(s). "
                             ...
                             "This is typically used for SDS based apps like CallService/iCampaign for"
                             "based messages involving Camel/IN/MAP events")
